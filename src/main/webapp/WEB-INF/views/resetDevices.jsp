                            ...
                            <form  id="formreset"   >
                                <fieldset>
                                    ...
                                    <div style="margin-left:70px;border:1px solid white;width:500px;height:380px;">
                                        ...
                                        <div style="float: left;">
                                            <label >GPS信息</label>  <input name="gps" id="gps" style="width:300px;margin-left: 50px; " placeholder="填写已激活" type="text">
                                        </div>
                                        <div style="float: left;">
                                            <label >客户控制端2</label>  <input name="controllerid2" id="controllerid2"style="width:300px; margin-left: 27px;" placeholder="填写客户注册手机号" type="text">
                                        </div>
                                        <div style="float: left;">
                                            <label >绑定状态2</label>  <input name="devicebinding2" id="devicebinding2"style="width:300px;margin-left: 40px; " placeholder="绑定状态,是则填true,否则填false" type="text">
                                        </div>
...
<%--设备数据提交--%>
<script>
  /*  var serverurl="http://192.168.1.100:9010/user/resetdevices";
   var serverurlresetDevices="http://192.168.1.100:9010/user/resetdevices";*/

    var serverurl="http://139.224.47.78:9010/user/resetdevices";
    var serverurlresetDevices="http://139.224.47.78:9010/user/resetdevices";


    function setdefault() {
        var devicesid = $('#devicesid').val();

        $("#cardnumber").val("粤EJ88888");
        $("#company").val("天涯同行有限公司");
        $("#controllerid").val("13425144857");
        $("#devicebinding").val("true");
        $("#devicepassword").val("12345678");
        $("#onlinestatus").val("掉线");
        $("#activates").val("已激活");

        $("#gps").val("DID"+devicesid+"$GPRMC,085746.00,A,2242.24931,N,11349.47541,E,0.724,,251116,,,A*7F");
        $("#controllerid2").val("00");
        $("#devicebinding2").val("false");
    ...
    function updata() {
        ...
        var devicebinding = $('#devicebinding').val();
        var onlinestatus = $('#onlinestatus').val();
        var company = $('#company').val();
        var cardnumber = $('#cardnumber').val();
        var activate = $('#activates').val();

        var gps=$('#gps').val();
        var controllerid2 = $('#controllerid2').val();
        var devicebinding2 = $('#devicebinding2').val();
        ...
        $.ajax({
            ...
            jsonpCallback:'updateDevices',                   //回调函数名

            success:function(result){                  //成功执行处理，对应后台返回的getName(data)方法。
                //  alert(result.update);
                if(result.updataup=="upsucceed")
                {
                    alert("设备更新成功");
                }else {
                    alert("设备更新失败");
    ...
    $(function() {
        ...
        var ok4 = false;
        // 验证用户名
        $('input[name="devicesid"]').focus(function () {
            $(this).next().text('8字节').removeClass('state1').addClass('state2');
        }).blur(function () {
            ...
            if ($(this).val().length ==8 && $(this).val() != '') {
                ...
                $.ajax({
                    url:serverurlresetDevices,       //跨域到http://www.wp.com，另，http://test.com也算跨域

                    type:'GET',                                //jsonp 类型下只能使用GET,不能用POST,这里不写默认为GET
                    dataType:'jsonp',                          //指定为jsonp类型
                    data:{"devicesid":contentid},                //数据参数
                    jsonp:'callback',                          //服务器端获取回调函数名的key，对应后台有$_GET['callback']='getName';callback是默认值
                    jsonpCallback:'resetDevices',                   //回调函数名

                    success:function(result){                  //成功执行处理，对应后台返回的getName(data)方法。
                        // console.log('ajax返回数据：'+result.rows.toString()+result.total.toString());
                        ...
                        $.each(result.rows, function (i, row) {
                            ...
                            if(row.deviceid==contentid)
                            {
                                a=1;
                                console.log("自动赋值");
                                $("#cardnumber").val(row.cardnumber);
                                $("#company").val(row.company);
                                $("#controllerid").val(row.controllerid);
                                $("#devicebinding").val(row.devicebinding);
                                $("#devicepassword").val(row.devicepassword);
                                $("#onlinestatus").val(row.onlinestatus);
                                $("#activates").val(row.activate);

                                $("#gps").val(row.gps);
                                $("#controllerid2").val(row.controllerid2);
                                $("#devicebinding2").val(row.devicebinding2);

                            }
//                            console.log("对象"+i+row.company+";当前对象数据："+contentid);
                        });
                        if(a==0)
                        {
                            $("#cardnumber").val("");
                ...
                $(this).next().text('请输入编号').removeClass('state1').addClass('state3');
            }
